...
model2owl project's README](https://github.com/OP-TED/model2owl/blob/develop/README.md#generating-diff-reports).
...
The workflow trigger is specified based on the following files that can be changed:
```yaml
    paths:
      - 'implementation/demo_ontology/owl_ontology/demo_ontology_CM.ttl'
      - 'implementation/demo_ontology/shacl_shapes/demo_ontology_CM_shapes.ttl'
```
If any change is detected in the files that are included in the above files, it
will trigger this GitHub action. The paths refer to artefacts automatically
generated by the [Transform with model2owl](#transform-with-model2owl) workflow
that is triggered on changes to the input XMI file(s).

User should adjust configuration values by defining [Github
Variables](https://docs.github.com/en/actions/how-tos/write-workflows/choose-what-workflows-do/use-variables#creating-configuration-variables-for-a-repository)
on the dedicated [project
page](https://github.com/OP-TED/model2owl-boilerplate/settings/variables/actions).

#### Workflow configuration parameters
The following list describes available configuration parameters:

- `REVISION_TO_COMPARE_COMMIT_ID` - (Optional) Specification of a Git revision (branch, tag or SHA) for the old version
- `REVISION_TO_COMPARE_REPO_URL` - (Optional) URL of an external public Git repository for the old version
- `OLD_ONTOLOGY_FILE` - Specification of the old version; a path to an OWL core file
- `OLD_SHACL_FILE` - Specification of the old version; a path to a SHACL shapes file
- `NEW_ONTOLOGY_FILE` - Specification of the new version; a path to an OWL core file
- `NEW_SHACL_FILE` - Specification of the new version; a path to a SHACL shapes file
- `RDF_DIFF_OUTDIR` - Output directory for generated diffing reports

The paths specified in `OLD_ONTOLOGY_FILE` and `OLD_SHACL_FILE` are resolved
according to values of `REVISION_TO_COMPARE_COMMIT_ID` and
`REVISION_TO_COMPARE_REPO_URL` parameters. The following cases are supported by
the workflow:
- `REVISION_TO_COMPARE_COMMIT_ID` and `REVISION_TO_COMPARE_REPO_URL` specified:
  an RDF file is fetched from an external Git repository, following the
  specified revision and path.
- `REVISION_TO_COMPARE_COMMIT_ID` specified: an RDF file is fetched from the
  same Git repository, following the specified revision and path.
- `REVISION_TO_COMPARE_COMMIT_ID` and `REVISION_TO_COMPARE_REPO_URL` not
  specified: an RDF file is fetched from the same Git repository, following the
  specified path. The current revision (the one that the workflow runs on) is
  used.
...
### Commit messages for automatically generated diff reports
The [RDF diffing](#rdf-diffing) workflow commits generated files to the
repository. It attaches information about the version of the artefacts used in
the commit message, which helps to identify the input files used for a
particular report file. The commit message is formulated according to a specific
convention. The convention is described below using an exemplary commit message.
```
chore(ci-auto): add RDF diff reports (ePO@release/5.0.0: ePO_core.ttl+ePO_core_shapes.ttl vs current: demo_ontology_CM.ttl+demo_ontology_CM_shapes.ttl)
```
The message encodes the following information referring to either *old* or *new*
version of the compared artefact:
* [old] identifier of an external Git repository (if
  `REVISION_TO_COMPARE_REPO_URL` was specified); example: `ePO`
* [old] identifier of a revision in the current/external repository (if
  `REVISION_TO_COMPARE_COMMIT_ID` was specified); example: `release/5.0.0`
* [old] name of the old OWL core file; example: `ePO_core.ttl`
* [old] name of the old SHACL shapes file; example: `ePO_core_shapes.ttl`
* [new] name of the new OWL core file; example: `demo_ontology_CM.ttl`
* [new] name of the new SHACL shapes file; example:
  `demo_ontology_CM_shapes.ttl`

Note: For brevity, the commit message uses file names rather than full paths.
